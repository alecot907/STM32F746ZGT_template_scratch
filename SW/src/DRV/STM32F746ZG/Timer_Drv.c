...
void Timer_Drv_Init (TIMER_LIST_t timer_list)
{
	...
	reg_temp = (Timer_Drv_Regs[timer_list].CCMR1_CaptComp1Selection << TIM_CCMR1_CC1S_Pos) |
							...
							((Timer_Drv_Regs[timer_list].CCMR1_OutComp1Mode & 0x7U) << TIM_CCMR1_OC1M_Pos) | (((Timer_Drv_Regs[timer_list].CCMR1_OutComp1Mode >> 0x3U) & 0x1U) << 16U);
	MODIFY_REG(timer->CCMR1, 0xFFFF, reg_temp);
	

	reg_temp = (Timer_Drv_Regs[timer_list].CCER_CaptComp1OutPol << TIM_CCER_CC1P_Pos) |
...
void Timer_Drv_Int (TIMER_LIST_t timer_list)
{
	...
	if (TIMER_ENABLE == Timer_Drv_Regs[timer_list].Int_Enable)
	{
		TIM_TypeDef *timer = Timer_Drv_Regs[timer_list].timer;
